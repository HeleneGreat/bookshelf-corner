...
<section class="container text-center bg-contact">
    ...
    <form action="index.php?action=createUserPost" method="post" enctype='multipart/form-data'>
        <p><input type="text" name="userPseudo" id="pseudo" placeholder="Votre pseudo" pattern="^[a-zA-Z0-9]{5,}$" title="Votre pseudo doit contenir au moins 5 caractère et n'accepte pas les caractères spéciaux" required><span class="required">*</span></p>
        <p><input type="email" name="userMail" id="mail" placeholder="Votre adresse e-mail" required><span class="required">*</span></p>
        <p><input type="password" name="userMdp" id="userMdp" placeholder="Votre mot de passe" pattern="^(?=.{7,})(?=.*[a-z])(?=.*[A-Z])(?=.*[!£@$%^&*()_+=\-`{}:~#';<>?/.,|\\]).*$" title="Votre mot de passe doit contenir au moins 7 caractères, une minuscule, une majuscule et un caractère spécial"><span class="required" required>*</span></p>
